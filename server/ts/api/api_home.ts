...
export const initHomeApi = () => {
	...
	app.get('/api/home/info', async (req: express.Request, res: express.Response) => {
		res.send(await getHomeInfo());
	});
};

/** Collects the level lists shown on the home page. Also used to server-side render it. */
export const getHomeInfo = async (): Promise<HomeInfo> => {
	...
	return { allLevels, filteredLevels };
};
