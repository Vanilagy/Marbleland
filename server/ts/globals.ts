...
export let keyValue: KeyValueStore<{ levelId: number, accountId: number, packId: number }>;
export let db: {
	missions: Datastore,
	accounts: Datastore,
	packs: Datastore
...
export const initGlobals = () => {
	keyValue = new KeyValueStore(path.join(__dirname, 'storage/keyvalue.json'), { levelId: 0, accountId: 0, packId: 0 });
